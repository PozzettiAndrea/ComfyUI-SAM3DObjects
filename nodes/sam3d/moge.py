...
- MoGe v2: DINOv2Encoder, Resampler, MLP, ConvStack, MoGeModel (model.pt-based)
...
def _make_dinov2_model_name(arch_name, patch_size, num_register_tokens=0):
    compact = {"vit_small": "vits", "vit_base": "vitb", "vit_large": "vitl", "vit_giant2": "vitg"}
    name = f"dinov2_{compact[arch_name]}{patch_size}"
    if num_register_tokens:
        name += f"_reg{num_register_tokens}"
    return name
...
def _make_dinov2_model(
    ...
    **kwargs,
    ...
    model = _ARCH_REGISTRY[arch_name](dtype=dtype, device=device, operations=operations, **vit_kwargs)
...
def dinov2_vits14(*, pretrained=True, dtype=None, device=None, operations=ops, **kwargs):
    return _make_dinov2_model(arch_name="vit_small", pretrained=pretrained, dtype=dtype, device=device, operations=operations, **kwargs)

def dinov2_vitb14(*, pretrained=True, dtype=None, device=None, operations=ops, **kwargs):
    return _make_dinov2_model(arch_name="vit_base", pretrained=pretrained, dtype=dtype, device=device, operations=operations, **kwargs)

def dinov2_vitl14(*, pretrained=True, dtype=None, device=None, operations=ops, **kwargs):
    return _make_dinov2_model(arch_name="vit_large", pretrained=pretrained, dtype=dtype, device=device, operations=operations, **kwargs)

def dinov2_vitg14(*, pretrained=True, dtype=None, device=None, operations=ops, **kwargs):
    return _make_dinov2_model(arch_name="vit_giant2", ffn_layer="swiglufused", pretrained=pretrained, dtype=dtype, device=device, operations=operations, **kwargs)

def dinov2_vits14_reg(*, pretrained=True, dtype=None, device=None, operations=ops, **kwargs):
    return _make_dinov2_model(arch_name="vit_small", pretrained=pretrained, num_register_tokens=4, interpolate_antialias=True, interpolate_offset=0.0, dtype=dtype, device=device, operations=operations, **kwargs)

def dinov2_vitb14_reg(*, pretrained=True, dtype=None, device=None, operations=ops, **kwargs):
    return _make_dinov2_model(arch_name="vit_base", pretrained=pretrained, num_register_tokens=4, interpolate_antialias=True, interpolate_offset=0.0, dtype=dtype, device=device, operations=operations, **kwargs)

def dinov2_vitl14_reg(*, pretrained=True, dtype=None, device=None, operations=ops, **kwargs):
    return _make_dinov2_model(arch_name="vit_large", pretrained=pretrained, num_register_tokens=4, interpolate_antialias=True, interpolate_offset=0.0, dtype=dtype, device=device, operations=operations, **kwargs)

def dinov2_vitg14_reg(*, pretrained=True, dtype=None, device=None, operations=ops, **kwargs):
    return _make_dinov2_model(arch_name="vit_giant2", ffn_layer="swiglufused", pretrained=pretrained, num_register_tokens=4, interpolate_antialias=True, interpolate_offset=0.0, dtype=dtype, device=device, operations=operations, **kwargs)

# Lookup table for hub-style loading (used by Dino class and MoGe)
_HUB_REGISTRY = {
...
def get_dinov2_backbone(name, pretrained=False, dtype=None, device=None, operations=ops, **kwargs):
    """Public API: create a DINOv2 backbone by name."""
    if name in _HUB_REGISTRY:
        return _HUB_REGISTRY[name](pretrained=pretrained, dtype=dtype, device=device, operations=operations, **kwargs)
...
class MoGeModelV1(nn.Module):
    ...
    def __init__(
        ...
        **deprecated_kwargs
        ...
        self.backbone = get_dinov2_backbone(encoder, pretrained=False, dtype=dtype, device=device, operations=operations)
...
# ==========================================================================
# MoGe v2 — Depth estimation model (model.pt-based)
# ==========================================================================

class ResidualConvBlockV2(nn.Module):
    """Residual conv block used in MoGe v2 ConvStack."""
    def __init__(
        self, in_channels, out_channels=None, hidden_channels=None,
        kernel_size=3, padding_mode='replicate',
        activation='relu',
        in_norm='layer_norm', hidden_norm='group_norm',
        dtype=None, device=None, operations=ops,
    ):
        super().__init__()
        if out_channels is None:
        ...
        act_map = {
            'relu': nn.ReLU, 'leaky_relu': functools.partial(nn.LeakyReLU, negative_slope=0.2),
        ...
        def _make_norm(norm_type, channels):
            if norm_type == 'group_norm':
                return operations.GroupNorm(channels // 32, channels, dtype=dtype, device=device)
            elif norm_type == 'layer_norm':
                return operations.GroupNorm(1, channels, dtype=dtype, device=device)
...
class DINOv2Encoder(nn.Module):
    """Wrapped DINOv2 encoder for MoGe v2. Input is RGB image in range [0, 1]."""
    backbone: DinoVisionTransformer
    image_mean: torch.Tensor
    image_std: torch.Tensor

    def __init__(self, backbone, intermediate_layers, dim_out,
                 dtype=None, device=None, operations=ops, **deprecated_kwargs):
        super().__init__()
        self.intermediate_layers = intermediate_layers
        self.backbone_name = backbone
        self.backbone = get_dinov2_backbone(backbone, pretrained=False, dtype=dtype, device=device, operations=operations)
        self.dim_features = self.backbone.embed_dim
    ...
    def init_weights(self):
        pretrained = get_dinov2_backbone(self.backbone_name, pretrained=True)
    ...
    def enable_pytorch_native_sdpa(self):
        ...
        if isinstance(blocks[0], BlockChunk):
            for chunk in blocks:
                for blk in chunk:
                    if hasattr(blk, 'attn'):
                        wrap_dinov2_attention_with_sdpa(blk.attn)
        else:
            for blk in blocks:
                if hasattr(blk, 'attn'):
                    wrap_dinov2_attention_with_sdpa(blk.attn)
...
class Resampler(nn.Sequential):
    def __init__(self, in_channels, out_channels,
                 type_='pixel_shuffle', scale_factor=2,
                 dtype=None, device=None, operations=ops):
...
class MLPV2(nn.Sequential):
    """Simple MLP used in MoGe v2 scale head."""
    def __init__(self, dims, dtype=None, device=None, operations=ops):
        nn.Sequential.__init__(self,
            *itertools.chain(*[
...
class ConvStack(nn.Module):
    def __init__(
        self,
        dim_in, dim_res_blocks, dim_out, resamplers,
        dim_times_res_block_hidden=1, num_res_blocks=1,
        ...
        self.resamplers = nn.ModuleList([
            Resampler(dim_prev, dim_succ, scale_factor=2, type_=resampler_, dtype=dtype, device=device, operations=operations)
            for dim_prev, dim_succ, resampler_ in zip(
                dim_res_blocks[:-1], dim_res_blocks[1:],
...
class MoGeModelV2(nn.Module):
    ...
    def forward(self, image, num_tokens):
        ...
        points, normal, mask = (
            F.interpolate(v.float(), (img_h, img_w), mode='bilinear', align_corners=False, antialias=False) if v is not None else None
    ...
    def infer(self, image, num_tokens=None, resolution_level=9,
        ...
        # No autocast — caller controls dtype
        ...
        if points is not None:
            ...
            fx = focal / 2 * (1 + aspect_ratio ** 2) ** 0.5 / aspect_ratio
            fy = focal / 2 * (1 + aspect_ratio ** 2) ** 0.5
            intrinsics = utils3d.torch.intrinsics_from_focal_center(fx, fy, torch.tensor(0.5, device=points.device), torch.tensor(0.5, device=points.device))
        ...
        return_dict = {'points': points, 'intrinsics': intrinsics, 'depth': depth, 'mask': mask_binary, 'normal': normal}
...
# Backward compatibility: MoGeModel = MoGeModelV1 (used by depth_estimate.py)
