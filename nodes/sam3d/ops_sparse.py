"""
ComfyUI-native sparse operations for SAM3DObjects.

Mirrors comfy/ops.py: provides `disable_weight_init` and `manual_cast` tiers
for sparse layers operating on SparseTensor.
...
from comfy.ops import cast_bias_weight, uncast_bias_weight, run_every_op
...
def _detect_available_conv_backend() -> str:
    ...
    # Also check legacy SPARSE_BACKEND env var
    env_backend = os.environ.get('SPARSE_BACKEND')
    if env_backend:
        valid_backends = ['none', 'spconv', 'torchsparse']
        if env_backend in valid_backends:
            log.info(f"Using conv backend from SPARSE_BACKEND env var: {env_backend}")
            return env_backend

    # Also check SPCONV_ALGO env var
    env_algo = os.environ.get('SPCONV_ALGO')
    if env_algo and env_algo in ['auto', 'implicit_gemm', 'native']:
        global SPCONV_ALGO
        SPCONV_ALGO = env_algo
...
def _spconv_conv3d_forward(self, x):
    ...
    if spatial_changed and (x.shape[0] != 1):
        # spconv with non-1 stride breaks contiguity — sort by batch index
...
class disable_weight_init:
    ...
    class SparseLinear(comfy.ops.disable_weight_init.Linear):
        """Linear that accepts SparseTensor: extract .feats, run linear, replace."""
    ...
    class SparseGroupNorm(comfy.ops.disable_weight_init.GroupNorm):
        """GroupNorm that handles SparseTensor with per-batch normalization."""
    ...
    class SparseLayerNorm(comfy.ops.disable_weight_init.LayerNorm):
        """LayerNorm that handles SparseTensor with per-batch normalization."""
...
class manual_cast(disable_weight_init):
    ...
    class SparseActivation(disable_weight_init.SparseActivation):
        pass
