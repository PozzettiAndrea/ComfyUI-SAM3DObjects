...
def copy_workflows():
    ...
    try:
        ...
        if not workflows_src.exists():
            print("[SAM3DObjects] No workflows/ directory found, skipping workflow copy")
            return
        ...
        for item in workflows_src.iterdir():
            ...
            # Destination path with SAM3DO- prefix
            dest = workflow_dir / f"SAM3DO-{item.name}"
            ...
            try:
                shutil.copy2(item, dest)
                print(f"[SAM3DObjects] Copied workflow: {item.name} -> {dest}")
                copied_count += 1
            except Exception as e:
                print(f"[SAM3DObjects] Failed to copy {item.name}: {e}")
        ...
        # Print summary
        if copied_count > 0:
            print(f"[SAM3DObjects] Copied {copied_count} workflow file(s) to {workflow_dir}")
        if skipped_count > 0:
            print(f"[SAM3DObjects] Skipped {skipped_count} existing workflow file(s)")

    except Exception as e:
        print(f"[SAM3DObjects] Error copying workflows: {e}")
